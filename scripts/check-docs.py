...
def check_index_covers_dir(index_path: str, dir_path: str, label: str) -> None:
    ...
    for file_name in md_files_in_dir:
        if f"({file_name})" not in index_content and f"/{file_name})" not in index_content:
            errors.append(f'{label}: "{file_name}" not listed in {index_path}')


check_index_covers_dir("docs/exec-plans/index.md", "docs/exec-plans/active", "exec-plans/active")
check_index_covers_dir("docs/exec-plans/index.md", "docs/exec-plans/completed", "exec-plans/completed")
    ...
    for f in full_dir.glob("*.md"):
        ...
        for field in REQUIRED_FRONTMATTER:
            if f"**{field}**" not in content:
                errors.append(f'{file_name}: missing required frontmatter field "{field}"')
        if is_active:
